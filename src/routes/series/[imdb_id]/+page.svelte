...
{#if series}
	...
	<div class="absolute top-14 flex flex-col h-screen">
		...
		<div class="m-2">
			...
			<div class="px-3 flex flex-row h-72">
				...
				<div class="w-2 h-2 rounded-full bg-white m-auto" />
				<p class="text-white text-sm">{series.runtime} min</p>
			</div>
			<p class="text-sm text-white">{series.synopsis}</p>

			<div class="flex flex-row">
				<div class="flex flex-col">
					{#each Array(series.seasonCount + 1).fill(0) as seasons, i}
						<button on:click={() => (openSeason = i + 1)}>Season {i + 1}</button>
					{/each}
				</div>

				<div class="flex flex-col overflow-x-scrol">
					{#if openSeason != null}
